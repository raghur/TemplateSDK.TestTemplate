 ...
 function Get-SingleDscOperation
 {
    ...
    if($JobId)
    {
        ...
        if($indexInArray -ge $groupedEvents.Count)
        {
            ...
            Log -Error "The Job ID Entered $JobId, does not exist among the dsc operations. To get a list of previously run DSC operations, run this command : Get-xDscOperation"
            return
        }
    }
...
#>
 ...
 function Trace-DscOperationInternal
 {
    ...
    {
        Log -Error "Please enter a valid Sequence ID . All sequence IDs can be seen after running command Get-xDscOperation . " -ForegroundColor Red
        return
    } 
...
Export-ModuleMember -Function Trace-xDscOperation, Get-xDscOperation